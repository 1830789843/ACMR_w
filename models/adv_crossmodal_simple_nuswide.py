...
# cPickle在python3.x中移除，直接是内置的包：pickle
import pickle
import os, time
import numpy as np
...
class DataIter(BaseDataIter):
    def __init__(self, batch_size):
        ...
        with open('data/nuswide/train_ids.pkl', 'rb') as f:
            self.train_ids = pickle.load(f, encoding='bytes')
        with open('data/nuswide/test_ids.pkl', 'rb') as f:
            self.test_ids = pickle.load(f, encoding='bytes')
        with open('data/nuswide/train_id_label_single.pkl', 'rb') as f:
            self.train_labels_single = pickle.load(f, encoding='bytes')
        with open('data/nuswide/test_id_label_single.pkl', 'rb') as f:
            self.test_labels_single = pickle.load(f, encoding='bytes')
    ...
    def eval(self, sess):
        ...
        for k in all_k: 
            for i in range(len(test_txt_vecs_trans)):
                ...
                # distances and sort by distances
                # 其中test_txt_vecs_trans的type是<class 'numpy.ndarray'>
                # 所以可以直接相减。见上面的定义
