...
class ModelParams(BaseModelParams):  # 模型参数
    def __init__(self):
    ...
    def train(self, sess):
        ...
        for epoch in range(self.model_params.epoch):
            ...
            for batch_feat, batch_vec, batch_labels, idx in self.data_iter.train_data():
                ...
                # 将每一个向量的数值分别减1：即（2，3，4）-（1，1，1）=（1，2，3）
                ...
                # b = shape(64,10)
                # transpose转置
                # adj_max = shape(64,64)单位阵
                adj_mat = np.dot(b, np.transpose(b))
                # mask
                # 返回的是对角元素均为0，其他均为1的矩阵
                ...
                # label_loss_val, dissimilar_loss_val, similar_loss_val = sess.run([total_train_op, self.label_loss,
                # self.dissimilar_loss, self.similar_loss], feed_dict={self.tar_img: batch_feat, self.tar_txt:
                # batch_vec, self.y: b, self.y_single: np.transpose([batch_labels]),self.l: l})
                ...
                label_loss_val, triplet_loss_val, emb_loss_val, domain_loss_val = sess.run(
                    ...
                    feed_dict={self.tar_img: batch_feat,
                               ...
                               self.y_single: np.transpose([batch_labels]),
                               self.l: l})
                print('Epoch: [%2d][%4d/%4d] time: %4.4f, emb_loss: %.8f, '
                      'domain_loss: %.8f, label_loss: %.8f, triplet_loss: %.8f'
